...
var parsedDate = function parsedDate(date) {
    return date.split('-').slice(0, 2).map(function (d) {
        return d > 12 ? d : month[d - 1];
    }).reverse().join(' ');
};

var formatTooltip = function formatTooltip(data) {
    var date = parsedDate(data[0]);
    return '$' + data[1] + ' Billion<br/><span class=\'date\'>' + date + '</span>';
};

var loadViz = function loadViz(json) {

    var data = json.data;

    var barWidth = 3;

    var scaled = d3.scaleLinear().domain([0, d3.max(data, function (d) {
        return d[1];
    })]).range([0, 480]);

    var div = d3.select('body').append('div').attr('class', 'tooltip').style('opacity', 0);

    var svg = d3.select('body').append('svg').attr('width', barWidth * data.length + 20).attr('height', 500);

    svg.append('text').attr('x', 30).attr('y', 50).text(json.source_name);

    svg.append('text').attr('x', 32).attr('y', 72).attr('class', 'gdp').text('Quarterly ' + json.name.split(',')[0] + ' (billions)');

    var bar = svg.selectAll('g').data(data).enter().append('g').attr('transform', function (d, i) {
        return 'translate(' + (i * barWidth + 10) + ', ' + (490 - scaled(d[1])) + ')';
    });

    bar.append('rect').attr('width', barWidth - 1).attr('height', function (d) {
        return scaled(d[1]);
    }).attr('fill', 'black').on('mouseover', function (d) {
        div.transition().duration(200).style('opacity', .9);
        div.html(formatTooltip(d)).style('left', d3.event.pageX + 'px').style('top', d3.event.pageY - 72 + 'px');
